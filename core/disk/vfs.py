...
class MemoryVFS(VirtualFileSystem):
    ...
    def save(self, path: str, content: str):
        ...
        log.debug(f"Saved file {path} ({len(content)} bytes) to memory")
    ...
    def remove(self, path: str):
        try:
            del self.files[path]
            log.debug(f"Removed file {path} from memory")
        except KeyError:
            log.warning(f"Attempted to remove non-existent file: {path}")
...
class LocalDiskVFS(VirtualFileSystem):
    ...
    def save(self, path: str, content: str):
        ...
        with open(full_path, "w", encoding="utf-8") as f:
            f.write(content)
        log.debug(f"Saved {path} ({len(content)} bytes)")
